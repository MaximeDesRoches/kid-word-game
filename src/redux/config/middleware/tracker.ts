...
const dictionary: { [key: string]: ActionStats } = {};
...
const tracker = (/*store*/) => (next: (arg0: any) => any) => (action: { type: string; }) => {
...
export const debugTracker = () => {
	return Object.keys(dictionary).reduce((c: { [key: string]: any }, v) => {
...
export const numberOfCalls = (filters:string[], secondFilters:string[] = []) => {
	return sum(
		filterActions(filters, secondFilters).map(action => dictionary[action].calls?.length || 0)
	);
};
